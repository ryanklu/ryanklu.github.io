...
;// FOH - App
var fohApp = angular.module('fohApp', [
    //'ngRoute',
    'ngSanitize',
    'fohControllers',
    'fohFilters',
...
// FOH - Config
// fohApp.config(['$locationProvider','$routeProvider', function($locationProvider, $routeProvider) {
//     $locationProvider.html5Mode(false);
// }]);

// FOH - Config
fohApp.run(['$rootScope', function ($rootScope) {
}]);;// FOH - Controllers
var fohControllers = angular.module('fohControllers', []);;// FOH - Directives
var fohDirectives = angular.module('fohDirectives', []);
fohDirectives.directive('backImg', function(){
...
var fohFilters = angular.module('fohFilters', []);;;var fohUtils = {
    init: function(){

    },

    ajaxArticleContent: function(sub_slide){
        var self = this;

        // For ajax Testing only
        var ajaxUrl = $(sub_slide).data('ajaxUrl');

        $(sub_slide).data('loaded', 'true');

        $.ajax({
            url: ajaxUrl,
            success: function(data){
                self.parseHTMLData(data, sub_slide);
            }
        });
    },

    parseHTMLData: function(data, sub_slide){
        var vidSections = ['#ContentPane5', '#ContentPane6', '#ContentPane7', '#ContentPane8'];
        var ssSections = ['#ContentPane9', '#ContentPane10', '#ContentPane11', '#ContentPane12'];
        var $data = $(data);

        var ajaxHtml = [];
        var vidIndex = 0, ssIndex = 0;

        vidSections.forEach(function(section){
            vidIndex++;
            ...
            ajaxHtml.push({
                assetType: 'video',
                placementSelector: '.vid' + vidIndex,
                content: html,
                subSlide: sub_slide
            });
        });

        ssSections.forEach(function(section){
            ssIndex++;
        ...
        this.placeHtml(ajaxHtml);

        // var html = '<img src="img/article-slideshow.jpg" style="width:100%;margin-bottom: 35px;" alt="">';

        // console.log($(sub_slide).find('.ss1 ss'));

        // $(sub_slide).find('.ss1 .ss').append(html);
    },

    placeHtml: function(ajaxHtml){
        ajaxHtml.forEach(function(sectionObj){
            if (sectionObj.content === ''){
                return;
            } else {
                console.log($(sectionObj.subSlide + ' ' + sectionObj.placementSelector));
            }
        });
    }
}
    ...
    $('.section_wrap').fullpage({
        ...
        afterSlideLoad: function(anchorLink, index, slideAnchor, slideIndex){
            ...
            if(document.location.hash.indexOf('/') > -1){
                ...
                if($('#sub_slide' + index).data('loaded')){
                    console.log('dont do it!!');
                } else {
                    fohUtils.ajaxArticleContent('#sub_slide' + index);
                }
            }
        },
        onSlideLeave: function(anchorLink, index, slideIndex, direction){
            //enable scrolling down and up
            $.fn.fullpage.setAllowScrolling(true, 'down, up');
        }
    });
